...
{   
  ...
  {          
    ...
    if (Math.abs(myDirectionX) < 20)
      myDirectionX = (float)(myDirectionX + ((dAmount) * Math.cos(dRadians)));
    if (Math.abs(myDirectionY) < 20)
      myDirectionY = (float)(myDirectionY + ((dAmount) * Math.sin(dRadians))); 
  }   
